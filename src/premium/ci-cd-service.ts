...
export class CICDService {
  ...
  async triggerBuild(
    provider: keyof CICDConfig['providers']
  ): Promise<BuildResult> {
    ...
    try {
      ...
      switch (provider) {
        case 'github':
          result = await this.triggerGitHubBuild(
            providerConfig as GitHubConfig
          );
          break;
        case 'gitlab':
          result = await this.triggerGitLabBuild(
            providerConfig as GitLabConfig
          );
          break;
        case 'netlify':
          result = await this.triggerNetlifyBuild(
            providerConfig as NetlifyConfig
          );
          break;
        case 'vercel':
          result = await this.triggerVercelBuild(
            providerConfig as VercelConfig
          );
          break;
        case 'custom':
          result = await this.triggerCustomBuild(
            providerConfig as CustomConfig
          );
          break;
        default:
      ...
      if (result.success) {
        this.outputChannel.appendLine(
          `✅ Build successful${result.url ? ` - ${result.url}` : ''}`
        );
      } else {
  ...
  private async triggerGitHubBuild(config: GitHubConfig): Promise<BuildResult> {
  ...
  private async triggerGitLabBuild(config: GitLabConfig): Promise<BuildResult> {
  ...
  private async triggerNetlifyBuild(
  ...
  private async triggerVercelBuild(config: VercelConfig): Promise<BuildResult> {
    // Deploy to Vercel
  ...
  private async triggerCustomBuild(config: CustomConfig): Promise<BuildResult> {
  ...
  async checkDeployment(
    url: string
  ): Promise<{ success: boolean; status: number; responseTime: number }> {
