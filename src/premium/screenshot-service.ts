...
export class ScreenshotService {
  ...
  async captureFullPageScreenshot(url: string, options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    return this.captureScreenshot(url, { ...options, fullPage: true });
  }

  async captureElementScreenshot(url: string, selector: string, options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    return this.captureScreenshot(url, { ...options, selector });
  }

  async captureResponsiveScreenshots(url: string, devices: Array<{ name: string; width: number; height: number }>): Promise<ScreenshotResult[]> {
  ...
  async openScreenshotManager(): Promise<void> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
  ...
  async createScreenshotGallery(): Promise<string> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
...
<body>
  ...
  private generateScreenshotGuide(url: string, options: ScreenshotOptions): string {
