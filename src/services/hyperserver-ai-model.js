...
exports.HyperServerAIModelManager = void 0;
class HyperServerAIModelManager {
    ...
    async ensureToolAvailable(toolId) {
        ...
        if (tool.bundled) {
            ...
            this.outputChannel.appendLine(`✅ Tool ${tool.name} is available via npm`);
            return true;
        }
        return false;
    }
    async analyzeCodeWithTool(toolId, code, task) {
        const tool = this.tools.get(toolId);
        if (!tool) {
    ...
    analyzeWithPatterns(code, task, toolType) {
        switch (toolType) {
            case 'code-analysis':
    ...
    analyzeCodePatterns(code) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('var ')) {
                issues.push(`Line ${lineNum}: Use 'let' or 'const' instead of 'var' for better scoping`);
            }
            // Check for console.log in production code
            if (line.includes('console.log(')) {
                issues.push(`Line ${lineNum}: Remove console.log before production - use proper logging`);
            }
            // Check for loose equality
            if (line.includes(' == ') && !line.includes(' === ')) {
                issues.push(`Line ${lineNum}: Use strict equality (===) instead of loose equality (==)`);
            }
            // Check for eval usage
            if (line.includes('eval(')) {
                issues.push(`Line ${lineNum}: Avoid eval() for security reasons - use safer alternatives`);
            }
            // Check for function spacing
            if (line.includes('function(') && !line.includes('function (')) {
                issues.push(`Line ${lineNum}: Add space after 'function' keyword for consistency`);
            }
            ...
            if (line.trim() &&
                ...
                if (line.includes('return ') ||
                    ...
                    issues.push(`Line ${lineNum}: Consider adding semicolon for consistency`);
                }
            }
            // Check for long lines
            if (line.length > 80) {
                issues.push(`Line ${lineNum}: Line is ${line.length} characters long - consider breaking it up`);
            }
        }
        // Check for overall code structure
        if (!code.includes('use strict') && code.includes('function')) {
            issues.push('Consider adding "use strict" directive for better error checking');
        }
        if (code.includes('setTimeout') && code.includes('0')) {
            issues.push('Consider using requestAnimationFrame instead of setTimeout(0) for better performance');
        }
        if (issues.length > 0) {
    ...
    analyzeAccessibilityPatterns(code) {
        const issues = [];
        const lines = code.split('\n');
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('<img') && !line.includes('alt=')) {
                issues.push(`Line ${lineNum}: Image missing alt text - add alt="description" for screen readers`);
            }
            // Check for buttons without accessible text
            if (line.includes('<button') &&
                !line.includes('>') &&
                !line.includes('aria-label=')) {
                issues.push(`Line ${lineNum}: Button missing accessible text - add text content or aria-label="description"`);
            }
            ...
            if ((line.includes('<input') ||
                ...
                issues.push(`Line ${lineNum}: Form control missing label - add <label> or aria-label="description"`);
            }
            // Check for heading hierarchy
            if (line.match(/<h[1-6]/)) {
                const headingLevel = parseInt(line.match(/<h([1-6])/)?.[1] || '1');
                if (headingLevel > 1 && !code.includes(`<h${headingLevel - 1}`)) {
                    issues.push(`Line ${lineNum}: Heading hierarchy issue - h${headingLevel} should follow h${headingLevel - 1}`);
                }
            }
            ...
            if (line.includes('color:') &&
                (line.includes('red') ||
                    line.includes('green') ||
                    line.includes('blue'))) {
                issues.push(`Line ${lineNum}: Color-only information - ensure information isn't conveyed by color alone`);
            }
            // Check for keyboard navigation
            if (line.includes('onclick=') && !line.includes('tabindex=')) {
                issues.push(`Line ${lineNum}: Interactive element should be keyboard accessible - add tabindex="0"`);
            }
        }
        // Check for overall document structure
        if (!code.includes('<main>') && !code.includes('<article>')) {
            issues.push('Document missing semantic landmarks - add <main> or <article> for better navigation');
        }
        if (!code.includes('lang=')) {
            issues.push('HTML missing language attribute - add lang="en" to <html> tag');
        }
        if (issues.length > 0) {
    ...
    analyzePerformancePatterns(code) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('querySelector') &&
                ...
                issues.push(`Line ${lineNum}: DOM queries in loops are expensive - cache selectors outside the loop`);
            }
            // Check for missing event listener cleanup
            if (line.includes('addEventListener') &&
                !code.includes('removeEventListener')) {
                issues.push(`Line ${lineNum}: Consider removing event listeners to prevent memory leaks`);
            }
            // Check for slow DOM manipulation
            if (line.includes('innerHTML') || line.includes('outerHTML')) {
                issues.push(`Line ${lineNum}: innerHTML/outerHTML can be slow - consider using textContent or DOM manipulation`);
            }
            // Check for setTimeout(0) anti-pattern
            if (line.includes('setTimeout') && line.includes('0')) {
                issues.push(`Line ${lineNum}: setTimeout(0) can be replaced with requestAnimationFrame for better performance`);
            }
            // Check for synchronous operations
            if (line.includes('alert(') ||
                line.includes('confirm(') ||
                line.includes('prompt(')) {
                issues.push(`Line ${lineNum}: Synchronous operations block the UI - consider async alternatives`);
            }
            // Check for large loops
            if (line.includes('for(') &&
                code.includes('length') &&
                code.includes('i++')) {
                issues.push(`Line ${lineNum}: Large loops can block the main thread - consider chunking or Web Workers`);
            }
        }
        // Check for overall performance patterns
        if (code.includes('document.write(')) {
            issues.push('document.write() blocks rendering - use DOM manipulation instead');
        }
    ...
    analyzeSEOPatterns(code) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('<title>') && line.trim() === '<title></title>') {
                issues.push(`Line ${lineNum}: Title tag is empty - add descriptive title for SEO`);
            }
            // Check for missing meta description
            if (line.includes('meta name="description"') &&
                line.includes('content=""')) {
                issues.push(`Line ${lineNum}: Meta description is empty - add descriptive content for search results`);
            }
            // Check for missing h1 tag
            if (line.includes('<h1>') && line.trim() === '<h1></h1>') {
                issues.push(`Line ${lineNum}: H1 tag is empty - add main heading for SEO hierarchy`);
            }
            // Check for images without alt text
            if (line.includes('<img') && !line.includes('alt=')) {
                issues.push(`Line ${lineNum}: Image missing alt text - helps with image search and accessibility`);
            }
            ...
            if (line.includes('<div>') &&
                ...
                issues.push(`Line ${lineNum}: Consider using semantic HTML elements (main, article, section) for better SEO`);
            }
            // Check for missing language attribute
            if (line.includes('<html') && !line.includes('lang=')) {
                issues.push(`Line ${lineNum}: HTML missing language attribute - add lang="en" for better SEO`);
            }
            // Check for missing viewport meta tag
            if (line.includes('<head>') && !code.includes('viewport')) {
                issues.push(`Line ${lineNum}: Missing viewport meta tag - add for mobile SEO`);
            }
        }
        ...
        if (code.includes('<div>') &&
            !code.includes('<main>') &&
            !code.includes('<article>')) {
            issues.push('Consider using semantic HTML elements (main, article, section)');
        }
        if (issues.length > 0) {
    ...
    analyzeSecurityPatterns(code) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('innerHTML') || line.includes('outerHTML')) {
                issues.push(`Line ${lineNum}: Potential XSS vulnerability - use textContent instead of innerHTML`);
            }
            // Check for eval usage
            if (line.includes('eval(')) {
                issues.push(`Line ${lineNum}: Critical security risk - eval() can execute arbitrary code`);
            }
            // Check for inline event handlers
            if (line.includes('onclick=') ||
                line.includes('onload=') ||
                line.includes('onerror=')) {
                issues.push(`Line ${lineNum}: Inline event handlers can be security risks - use addEventListener`);
            }
            ...
            if (line.includes('password') ||
                ...
                line.includes('token')) {
                if (line.includes('"') || line.includes("'")) {
                    issues.push(`Line ${lineNum}: Hardcoded secret detected - move to environment variables`);
                }
            }
            // Check for localStorage with sensitive data
            if (line.includes('localStorage') &&
                (line.includes('password') || line.includes('token'))) {
                issues.push(`Line ${lineNum}: Sensitive data in localStorage - use secure storage or encryption`);
            }
            // Check for SQL injection patterns
            if (line.includes('query') && line.includes('+') && line.includes('"')) {
                issues.push(`Line ${lineNum}: Potential SQL injection - use parameterized queries`);
            }
            // Check for unsafe redirects
            if (line.includes('window.location') &&
                line.includes('href') &&
                line.includes('+')) {
                issues.push(`Line ${lineNum}: Unsafe redirect - validate and sanitize URLs`);
            }
            // Check for unsafe file operations
            if (line.includes('readFile') && !line.includes('path.join')) {
                issues.push(`Line ${lineNum}: Unsafe file path - use path.join() to prevent path traversal`);
            }
        }
        // Check for overall security patterns
        if (!code.includes('Content-Security-Policy')) {
            issues.push('Missing Content Security Policy - add CSP headers to prevent XSS');
        }
    ...
    analyzeLintingPatterns(code) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('if(') && !line.includes('if (')) {
                issues.push(`Line ${lineNum}: Add space after 'if' keyword for consistency`);
            }
            if (line.includes('for(') && !line.includes('for (')) {
                issues.push(`Line ${lineNum}: Add space after 'for' keyword for consistency`);
            }
            ...
            if (line.includes('"') && line.includes("'")) {
                issues.push(`Line ${lineNum}: Use consistent quote style - prefer single or double quotes`);
            }
            ...
            if (leadingSpaces % 2 !== 0 && leadingSpaces > 0) {
                issues.push(`Line ${lineNum}: Use consistent indentation (2 or 4 spaces)`);
            }
            // Check for long lines
            if (line.length > 80) {
                issues.push(`Line ${lineNum}: Line is ${line.length} characters long - consider breaking it up`);
            ...
            if (line.includes('import ') || line.includes('require(')) {
                const importName = line.match(/(?:import|require).*?['"`]([^'"`]+)['"`]/)?.[1];
                if (importName && !code.includes(importName.split('/').pop() || '')) {
                    issues.push(`Line ${lineNum}: Unused import detected - remove if not needed`);
                }
            }
        }
...
exports.HyperServerAIModelManager = HyperServerAIModelManager;
//# sourceMappingURL=hyperserver-ai-model.js.map