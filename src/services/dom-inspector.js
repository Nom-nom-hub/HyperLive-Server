...
class DOMInspector {
    ...
    async openInspector() {
        ...
        this.panel = vscode.window.createWebviewPanel('domInspector', 'DOM Inspector', vscode.ViewColumn.Two, {
            enableScripts: true,
    ...
    analyzeIssues(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (['button', 'input', 'select', 'textarea'].includes(node.tagName.toLowerCase())) {
            ...
            if (node.attributes.style) {
                issues.push({
                    ...
                    nodeId: node.id,
                    suggestion: 'Consider moving styles to external CSS for better performance',
                });
            }
        }
        return issues;
    }
    calculatePerformanceMetrics(nodes) {
        const depth = this.calculateMaxDepth(nodes);
        return {
            totalNodes: nodes.length,
            depth,
            renderTime: nodes.length * 0.1,
            memoryUsage: nodes.length * 0.5,
            repaints: Math.floor(nodes.length * 0.3),
            reflows: Math.floor(nodes.length * 0.1),
        };
    }
    calculateAccessibilityMetrics(nodes) {
    ...
    onNodeSelected(nodeId) {
        if (this.currentAnalysis) {
            ...
            if (node) {
                ...
                vscode.commands.executeCommand('advancedLiveServer.highlightNode', node);
            }
        }
    }
    getCurrentAnalysis() {
